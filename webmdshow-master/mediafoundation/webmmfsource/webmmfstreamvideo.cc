...
namespace WebmMfSourceLib
{
        ...
        IMFStreamDescriptor*& pDesc)
    {
        ...
        //BDR
        //https://msdn.microsoft.com/en-us/library/windows/desktop/aa370819%28v=vs.85%29.aspx
        ...
        //BDR
        //https://msdn.microsoft.com/en-us/library/windows/desktop/ms696277(v=vs.85).aspx 
        ...
        if (hr == S_OK)
        {
            hr =    (pmt, MF_MT_FRAME_RATE, numer, denom);
            assert(SUCCEEDED(hr));
        }
    ...
    HRESULT WebmMfStreamVideo::GetSample(IUnknown* pToken)
    {
        ...
        for (int i = 0; i < frame_count; ++i)
        {
            ...
            hr = MFCreateMemoryBuffer(360 * 640 * 3, &pBuffer);
            assert(SUCCEEDED(hr));
            assert(pBuffer);
            ...
            long status = 0;
            //status = f.Read(pReader, ptr);
            //BDR
            //BYTE* random = new BYTE[360*640*3]; 
            //CopyMemory(ptr, random, cbBuffer); 
            for (int c = 0; c < cbBuffer; c++)
            {
                ptr[c] = 0x4f; 
            }
            //delete random; 
            ...
            if (status >= 0)  //success
            {
                hr = pBuffer->SetCurrentLength(360 * 640 * 3);
                assert(SUCCEEDED(hr));
            }
